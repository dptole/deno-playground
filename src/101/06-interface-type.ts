...
*/
async function* createNumberGuessingGame(max: number): AsyncGenerator<GuessNumber> {
    const numberGenerated: number = Math.random() * max >> 0
    let totalAttempts: number = 0
    let diff: Diff = Diff.NAN

    console.log('A random number between 0 and ' + max + ' (inclusive) was generated')
...
for await (const guess of createNumberGuessingGame(1_000 /* Ruby/Elixir syntax */)) {
